...
def login_to_server(username, password):  # 接受用户名，密码字符串，返回一个socket对象，但是，
    ...
    data_txt = bytes(0 + ' ' + username + ' ' + password,
                     encoding='utf8')  # 消息的发送格式应该是二进制
    skt1.send(data_txt)
    ...
    while True:
        ...
        try:  # 二、收银员 msg = [ 2, x, ...]
            if msg[0] == '2':  # 返回  0——失败 1——成功
                if msg[1] == '1':  # 修改用户密码
                    # x = 1——id, old_password, new_password1
                    feedback = cashier.change_password(msg[2], msg[3], msg[4])
                if msg[1] == '2':  # 修改收银员信息
                    # x = 2——id, new_name, new_phone
                    feedback = cashier.change_info(msg[2], msg[3], msg[4])
                if msg[1] == '3':  # 注册客户信息，不返回0/1，因为一定成功。返回用户id
                    feedback = cashier.new_customer(
                        msg[2], msg[3])  # x = 3——name, phone
                if msg[1] == '4':  # 交易
                    # x = 4——customer_id, goods_id, quantity
                    feedback = cashier.purchase(msg[2], msg[3], msg[4])
                if msg[1] == '5':  # 购买VIP点数
                    # x = 5——customer_id, get_point
                    feedback = cashier.buy_point(msg[2], msg[3])
            if msg[0] == '3':  # 三、进货员 msg = [3, x, ...]
                if msg[1] == '1':  # 修改用户密码
                    # x = 1——id, old_password, new_password1
                    feedback = buyer.change_password(msg[2], msg[3], msg[4])
                if msg[1] == '2':  # 修改进货员信息
                    # x = 2——id, new_name, new_phone
                    buyer.change_info(msg[2], msg[3], msg[4])
                if msg[1] == '3':
            ...
            if msg[0] == '1':  # 四、管理员 msg = [1, x, ...]
                ...
                if msg[1] == '8':
                    feedback = administrator.modify_single_goods_info(
                        msg[2], msg[3], msg[4], msg[5], msg[6])
                if msg[1] == '9':
                    feedback = administrator.get_every_type_sum_profit(
                        msg[2], msg[3])
                if msg[1] == '10':
                    feedback = administrator.get_every_customer_sum_payment(
                        msg[2], msg[3])
                if msg[1] == '11':
